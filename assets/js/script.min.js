...
const SmoothScroll = {
  ...
  bindEvents() {
    ...
    this.lenis.on("scroll", ({ progress }) => {
      gsap.to(Config.selectors.scrollProgress, {
        scaleX: progress,
        duration: 0.1,
        ease: "none",
      });
...
const UI = {
  ...
  initTypewriter() {
    ...
    if (!el) return;
    const type = () => {
      const currentRole = Config.roles[state.roleIndex];
      if (state.isDeleting) {
        el.textContent = currentRole.substring(0, state.charIndex - 1);
        state.charIndex--;
      } else {
        el.textContent = currentRole.substring(0, state.charIndex + 1);
        state.charIndex++;
      ...
      if (!state.isDeleting && state.charIndex === currentRole.length) {
        ...
        setTimeout(type, Config.timing.newTextDelay);
      } else if (state.isDeleting && state.charIndex === 0) {
        state.isDeleting = false;
        state.roleIndex = (state.roleIndex + 1) % Config.roles.length;
        setTimeout(type, 500);
      } else {
        setTimeout(
          type,
          state.isDeleting ? Config.timing.eraseDelay : Config.timing.typeDelay,
        );
      }
    };
    type();
  ...
  initTiltCards() {
    ...
    cards.forEach((card) => {
      ...
      card.addEventListener("mousemove", (e) => {
        requestAnimationFrame(() => updateCard(e));
      });
      card.addEventListener("mouseleave", () => {
        isHovering = false;
        card.style.transform = "";
        card.style.setProperty("--glow-angle", "135deg");
  ...
  initMobileMenu() {
    ...
    const close = document.querySelector(Config.selectors.closeMenuBtn);
    const menu = document.querySelector(Config.selectors.mobileMenu);
    const links = document.querySelectorAll(Config.selectors.mobileLinks);
    if (!btn || !menu) return;
    const toggle = () => {
...
const Loader = {
  init() {
    ...
    this.initThreeLazyLoad();
    this.initLottieLazyLoad();
  },
  handleLoadingScreen() {
    window.addEventListener("load", () => {
      ...
      setTimeout(() => {
        ...
        }, Config.timing.loaderFadeTime);
      }, Config.timing.loaderMinTime);
    });
  },
  initThreeLazyLoad() {
  ...
  initLottieLazyLoad() {
    const lotties = document.querySelectorAll("lottie-player[data-src]");
    ...
    if ("IntersectionObserver" in window) {
      ...
      lotties.forEach((l) => observer.observe(l));
    } else {
...
const Scene3D = {
  ...
  createObjects() {
    ...
    this.crystalGeo = new THREE.IcosahedronGeometry(1.8, 1);
    this.crystalMat = new THREE.MeshBasicMaterial({
      color: 0x2dd4bf,
      wireframe: true,
      transparent: true,
      opacity: 0.12,
    });
    this.crystal = new THREE.Mesh(this.crystalGeo, this.crystalMat);
    this.scene.add(this.crystal);
    this.ringGeo = new THREE.TorusGeometry(3.5, 0.02, 16, 100);
  ...
  animate() {
    ...
    this.crystal.rotation.y += 0.003;
    this.crystal.rotation.x += 0.001;
    this.ring.rotation.z = elapsed * 0.1;
    ...
    this.crystalMat.opacity = 0.08 + pulse * 0.08;
...
const Animations = {
  ...
  heroReveal() {
    ...
    tl.fromTo(
      ".hero-title",
      { autoAlpha: 0, y: 150, scale: 0.8, rotateX: 15 },
      { autoAlpha: 1, y: 0, scale: 1, rotateX: 0, duration: 2.5, delay: 0.3 },
    ).fromTo(
      ".hero-content",
      { autoAlpha: 0, y: 40, filter: "blur(10px)" },
      { autoAlpha: 1, y: 0, filter: "blur(0px)", duration: 1.5 },
      "-=1.5",
    );
  },
  sectionReveals() {
    gsap.utils.toArray(".section-reveal").forEach((section) => {
      ...
      const label = section.querySelector(".font-technical");
      gsap.fromTo(
        section,
        { autoAlpha: 0 },
        {
  ...
  glassCards() {
    gsap.utils.toArray(".glass-card").forEach((card, i) => {
      gsap.fromTo(
        card,
        { autoAlpha: 0, y: 60, rotateX: 10, transformPerspective: 1000 },
        {
          scrollTrigger: {
            trigger: card,
            start: "top 85%",
  ...
  staggerItems() {
    if (document.querySelectorAll(".stagger-item").length) {
      gsap.set(".stagger-item", { autoAlpha: 0, y: 40, scale: 0.95 });
      ScrollTrigger.batch(".stagger-item", {
        ...
        onLeaveBack: (batch) =>
          gsap.to(batch, { autoAlpha: 0, y: 40, scale: 0.95, duration: 0.4 }),
        start: "top 88%",
      });
    }
  },
  footerAnimations() {
    const el = document.querySelector("footer h2");
    if (el) {
      gsap.fromTo(
        el,
        { autoAlpha: 0, scale: 0.7, y: 100 },
        {
          scrollTrigger: {
            trigger: "footer",
            start: "top 90%",
  ...
  parallaxBlobs() {
    gsap.utils.toArray(".parallax-blob").forEach((blob) => {
      gsap.to(blob, {
        y: window.innerHeight * blob.getAttribute("data-speed"),
        ease: "none",
        scrollTrigger: {
          trigger: "body",
          start: "top top",
  ...
  footerSocials() {
    const links = document.querySelectorAll(".social-pill");
    if (links.length) {
      gsap.fromTo(
        links,
